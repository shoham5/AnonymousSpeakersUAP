...
class ProjectedGradientDescent:
    def __init__(self,
        ...
        self.device = device
        self.clip_min = torch.tensor(clip_values[-1], dtype=torch.float32, device=device)
    ...
    def evaluate(self, inputs, targets, batch_info):
        pass

    def _generate_batch(self, inputs, targets, batch_info):
        adv_x = inputs.clone()
        momentum = torch.zeros(inputs.shape)
        progress_bar = tqdm(range(self.max_iter), total=self.max_iter, desc='Batch {}/{}'.format(batch_info['cur'], batch_info['total']))
        self.loss_values = []
        for _ in progress_bar:
            adv_x = self._compute(adv_x, inputs, targets, momentum)
            progress_bar.set_postfix_str('Batch Loss: {:.6}'.format(self.loss_values[-1]))
        return adv_x
