...
from utils.general import get_instance, save_config_to_file, calculator_snr_direct
...
class UniversalAttack:
    ...
    def generate(self):
        adv_pert = torch.zeros((1, self.cfg['fs'] * self.cfg['num_of_seconds']))
        ...
        for epoch in range(self.cfg.epochs):
            ...
            for i_batch, (cropped_signal_batch, person_ids_batch) in progress_bar:
                ...
                adv_snr = calculator_snr_direct(src_array=cropped_signal_batch, adv_array=self.apply_perturbation(cropped_signal_batch, adv_pert))
                running_snr += adv_snr
    ...
    def apply_perturbation(self, input, adv_pert):
        return input + adv_pert
    ...
    def evaluate(self, adv_pert):
        running_loss = 0.0
        running_snr = 0.0
        progress_bar = tqdm(enumerate(self.train_loader), desc=f'Eval', total=len(self.val_loader))
        prog_bar_desc = 'Batch Loss: {:.6}, SNR: {:.6}'
        for i_batch, (cropped_signal_batch, person_ids_batch) in progress_bar:
            loss, running_loss = self.forward_step(adv_pert, cropped_signal_batch, person_ids_batch, running_loss)
            adv_snr = calculator_snr_direct(src_array=cropped_signal_batch,
                                            adv_array=self.apply_perturbation(cropped_signal_batch, adv_pert))
            running_snr += adv_snr
            progress_bar.set_postfix_str(prog_bar_desc.format(running_loss / (i_batch + 1),
                                                              running_snr / (i_batch + 1)))
